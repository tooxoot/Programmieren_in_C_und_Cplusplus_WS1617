...
void resize_image(pixel_rgb_t *in, pixel_rgb_t *out, image_size_t src_sz, image_size_t trgt_sz) {
  ...
  for (int y = 0; y < trgt_sz.height; y++){
    for (int x = 0; x < trgt_sz.width; x++){
        ...
        int r = (1.0 - dy) * q0r + dy * q1r;
        int g = (1.0 - dy) * q0g + dy * q1g;
        int b = (1.0 - dy) * q0b + dy * q1b;

        // Solves colour problem discribed above.
        if(r<0) r = 0;
        if(g<0) g = 0;
        if(b<0) b = 0;
...
int main(int argc, char **argv) {
  ...
  switch (c)
    ...
    // Initializes the required variables then loads the jpeg resizes it and saves it.

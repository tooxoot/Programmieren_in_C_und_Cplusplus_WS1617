...
class RingBuffer {
private:
  int count;          /**< Keeps track of the number of elements currently in the buffer.*/
  int max_capacity;   /**< The maximum number of elements.*/

  /**
   * TODO
   * As it turns out I implemented these position indices the wrong way arround.
   * In my implementation the front index is pointing to an empty element not the back index.
   */
